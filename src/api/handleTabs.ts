...
function createNewObj(obj: Windows, selectObj: SelectObj): Windows {
	...
	selectWindows.map((key: keyof typeof obj) => {
		...
		newObj[key].map((tab, i) => {
			...
			if (status != tab.userSelected)
				newObj[key][i] = { ...tab, userSelected: status }
		})
	})
...
function collectTheSameTabsOfOldAndNewWindow(
	oldWindow: Array<Tab & CustomProps>,
	newWindow: Array<Tab & CustomProps>
) {
	...
	newWindow.map((tab, i) => {
		...
		const rstBoolArr = monitorProps.map(
